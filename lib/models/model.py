...
class Informer(nn.Module):
    def __init__(self, enc_in, dec_in, c_out, seq_len, label_len, out_len,
        ...
        self.pred_len = out_len  # 24
        self.attn = attn  # prob
        self.output_attention = output_attention  # False

        # Encoding
        self.enc_embedding = DataEmbedding(
            enc_in, d_model, embed, freq, dropout)  # 7, 512, timeF, h, 0.05
        self.dec_embedding = DataEmbedding(
            dec_in, d_model, embed, freq, dropout)  # 7, 512, timeF, h, 0.05
        # Attention
...
if __name__ == '__main__':
    model = Informer(enc_in=7, dec_in=7, c_out=7, seq_len=96, label_len=48, out_len=24,
                     factor=5, d_model=512, n_heads=8, e_layers=2, d_layers=1, d_ff=512,
                     dropout=0.05, attn='prob', embed='timeF', freq='h')
    x_enc = torch.rand((32, 96, 7))
    x_mark_enc = torch.rand((32, 96, 4))
    x_dec = torch.rand((32, 72, 7))
    x_mark_dec = torch.rand((32, 72, 4))
    outputs = model(
